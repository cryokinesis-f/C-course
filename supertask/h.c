//| Заголовок     | длина исходного файла (в байтах)         |
//| Дерево        | дерево Хаффмана (битово)                 |
//| Сжатые данные | поток битов — закодированный текст       |
//to comress ->>> ./h c <input.txt> <output.bit>
//to decompress ->>> ./h d <input.bin> <output.txt>
#include <stdio.h>   
#include <stdlib.h>
#include <stdint.h>
...
Node *make_node(void) {
    ...
    node->is_leaf = 0;                // считаем узел внутренним, если понадобится создать лист (символ), этот флаг можно изменить
    return node;
}
...
// --- построение таблицы кодов (для каждого символа последовательность бит) ---

typedef struct {
    unsigned int bits;    // код символа
...
void build_codes(Node *node, Code codes[], unsigned int code, int length) {
    ...
    // спускаемся налево, добавляя 0 к коду
    if (node->left)
...
void bw_init(Writer *bw, FILE *file) {
    bw->file = file;
    bw->buffer = 0;
    bw->bits_filled = 0;
}
...
void bw_write_bit(Writer *bw, int bit) {
...
void bw_write_bits(Writer *bw, unsigned int bits, int length) {
...
void br_init(Reader *br, FILE *file) {
    br->file = file;
    br->buffer = 0;
    br->bits_left = 0;
}
...
// --- Запись дерева Хаффмана в файл ---

void write_tree(Node *node, Writer *bw) {
    if (node->is_leaf) {
...
void compress_file(FILE *fin, FILE *fout) {
    ...
    // инициализируем Writer для записи битов
    Writer bw;
    bw_init(&bw, fout);
...
void decompress_file(FILE *fin, FILE *fout) {
    ...
    // инициализируем Reader для чтения битов
    Reader br;
    br_init(&br, fin);
